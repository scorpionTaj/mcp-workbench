...
import { Wrench, Package, Power, Settings, Trash2, ExternalLink, ChevronDown, ChevronRight, Code } from "lucide-react"
...
export default function ToolsPage() {
  const { servers, isLoading, toggleServer, uninstallServer } = useInstalledServers()
  ...
  const totalTools = installedServers.reduce((sum, s) => sum + s.tools.length, 0)
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground text-lg">Manage your installed MCP servers and available tools</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm text-muted-foreground">Installed Servers</p>
              <p className="text-2xl font-bold">{installedServers.length}</p>
            </div>
          </div>
        </Card>
      ...
      {isLoading ? (
        ...
        <Card className="p-12 text-center">
          ...
          <p className="text-muted-foreground text-sm mb-4">Visit the Registry to browse and install MCP servers</p>
          <Button asChild>
                ...
                <div className="p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      ...
                      {server.author && <p className="text-xs text-muted-foreground">by {server.author}</p>}
                    </div>
                    ...
                    <Dialog>
                      ...
                      <DialogContent>
                        <DialogHeader>
                          ...
                          <DialogDescription>
                            Server configuration options will be available here in a future update.
                          </DialogDescription>
                        </DialogHeader>
                      </DialogContent>
                    </Dialog>

                    <Button variant="destructive" size="sm" onClick={() => uninstallServer(server.id)}>
                  ...
                  {isExpanded && (
                    <div className="border-t border-border pt-4 space-y-3">
                      {server.tools.map((tool) => (
                        <div key={tool.name} className="bg-secondary/50 rounded-lg p-4">
                          <div className="flex items-start gap-3">
                            <div className="p-2 bg-primary/10 rounded-md">
                              <Code className="w-4 h-4 text-primary" />
                            </div>
                            <div className="flex-1">
                              <h4 className="font-medium text-sm mb-1 font-mono">{tool.name}</h4>
